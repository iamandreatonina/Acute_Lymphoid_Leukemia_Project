...
PACKAGES <- c(
  ...
  "smotefamily",     # Synthetic Minority Over-sampling Technique for imbalanced data
...
ML_data <- ML_data %>%
  ...
  select(-c(Cell_type, age, replicate, condition))  # Remove unnecessary columns
...
train_data <- AdasynClassif(type ~ ., train_data, beta = 0.1,k = 5,baseClass = "pediatric")
cli::cli_h3("\n[INFO] Train ML data after Adasyn:\n")
print(table(train_data$type))
train_data <- NCLClassif(type ~ ., train_data,k = 5) #,Cl = c("B", "PreT", "T"),
...
v_folds <- vfold_cv(train_data, v = 5, repeats = 3, strata = type)
...
knn_grid <- grid_space_filling(knn_param_space, size = 30, type = "latin_hypercube")
