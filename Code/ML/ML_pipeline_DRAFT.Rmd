...
Labels <- read.csv("C:/Users/perso/Desktop/Acute_Lymphoid_Leukemia_Project-main/data/ML_for_ALL/Labels.csv")
...
```{r}
# Find the row indices in df_original that have 'Unknown' labels
unknown_indices <- which(merged_df$Cell_type == 'Unkown')

NEW_ML <- ML_data

Predict_NN$Cell_type <- as.character(Predict_NN$Cell_type)

# Replace the 'Unknown' labels in df_original with the new classes from df_unknown_classified
NEW_ML$Cell_type[unknown_indices] <- Predict_NN$Cell_type


rm(ML_data_HS,ML_data_PCA,last_column)
```


```{r Plot PCA, message=FALSE, warning=FALSE}
ML_data_PCA$type <- NULL
ML_data_PCA$C_T <- NULL
ML_data_PCA$X.pam1_nonHS.clustering. <- NULL

# Create a 2D scatter plot using Plotly
fig2D <- plot_ly(ML_data_PCA, 
                 x = ~PC1, y = ~PC2,z = ~PC3, 
                 color = ML_data_PCA$Cell_type,
                 colors = brewer.pal(n = 4, name = "RdBu"),  
                 symbol = ML_data_PCA$Cell_type, 
...
library(Rtsne)
tsne_results <- Rtsne(ML_data[,-ncol(ML_data)],dims = 2)

tsne_data <- as.data.frame(tsne_results$Y)
rownames(tsne_data) <- rownames(ML_data)
colnames(tsne_data) <- c("tsne_1","tsne_2")
# Create a 2D scatter plot using Plotly
fig2D <- plot_ly(tsne_data, 
                 x = ~tsne_1, y = ~tsne_2, 
                 color = ML_data$Cell_type,
                 type = "scatter",
                 colors = brewer.pal(n = 4, name = "RdBu"),  
                 symbol = ML_data$Cell_type, 
                 symbols = c('diamond', 'circle', 'cross'), 
                 mode = 'markers')%>%
  layout(title = "nonHS T-SNE Plot")
...
umap_try <- umap(ML_data[,-ncol(ML_data)], config = custom.config)

umap_data <- as.data.frame(umap_try$layout)
colnames(umap_data) <- c("umap_1","umap_2")
rownames(umap_data) <- rownames(ML_data)
# Create a 2D scatter plot using Plotly
fig2D <- plot_ly(umap_data, 
                 x = ~umap_1, y = ~umap_2, 
                 type = "scatter",
                 color = ML_data$Cell_type,
                 colors = brewer.pal(n = 4, name = "RdBu"),  
...
my_data_train <- subset(ML_data, Cell_type != "Unkown")
Unkown_data <- subset(ML_data, Cell_type == "Unkown")
...
# Remove unnecessary objects
rm(model_RF,ctrl,var_imp,top_20)
```
...
tuneGrid <- expand.grid(kmax = 17:18, distance = c(0.45,0.55,0.65,0.75), kernel = c("optimal", "rectangular","gaussian"))
...
tuneGrid <- expand.grid(size = c(1,2,3,4), decay = c(0.1, 0.01, 0.001))
...
top_20 <- subset(var_imp, select = Overall)
top_20 <- top_20 %>% arrange(desc(Overall))
rownames(top_20) <- rownames(var_imp)
top_20 <- head(top_20,25)
