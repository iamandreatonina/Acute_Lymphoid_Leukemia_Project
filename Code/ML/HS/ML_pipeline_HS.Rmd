...
recipe <- recipe(Cell_type ~ ., data = train_data)%>%
  step_upsample(Cell_type,over_ratio = 0.5)
  #step_smotenc(Cell_type, over_ratio = 0.5) # Hybrid-sampling

# Perform cross-validation
cv_folds <- vfold_cv(train_data, strata = Cell_type, v = 5)
...
# Define the search grid for tuning
tree_grid <- grid_latin_hypercube(draft,size = 10)
...
tune_2 <- tune_sim_anneal(tuned_rf_workflow,resamples = cv_folds, iter = 50, initial = tune_1, metrics = class_metrics)
...
autoplot(confusion, type = "mosaic") +
  labs(title = "Confusion Matrix - Mosaic")
...
RF_var_importance <- 
  data.frame(ranger::importance_pvalues(rf_tree,method = "altmann", formula = Cell_type ~ ., data = train_data)) %>%
...
medium_threshold <- max(RF_var_importance$importance) * 0.25
...
ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  scale_fill_manual(values = c("lightpink", "lightgreen", "skyblue")) +  # Choose colors for each category
  labs(title = "Top 25 Variable Importance Plot RF", x = "Importance", y = "Variable")

# Make predictions
RF_results <- data.frame(predict(rf_fit, new_data = Unkown_data))
...
Knn_spec <- nearest_neighbor(weight_func = "gaussian",
              dist_power = tune(),
...
# Make predictions
KNN_results <- data.frame(predict(Knn_fit, new_data = Unkown_data))
...
tune1 <- tune_grid(tuned_xgb_workflow,resamples = cv_folds, grid = xgb_grid)

tune2 <- tune_sim_anneal(tuned_xgb_workflow, resamples = cv_folds, iter = 25, initial = tune1, metrics = class_metrics)
...
# View the confusion matrix
autoplot(confusion, type = "mosaic") +
  labs(title = "Confusion Matrix - Mosaic XGB") 

autoplot(confusion, type = "heatmap") +
  labs(title = "Confusion Matrix - Heatmap XGB")
...
# Make predictions
XGB_results <- data.frame(predict(xgb_fit, new_data = Unkown_data))
...
# Make predictions
mlp_results <- data.frame(predict(mlp_fit, new_data = Unkown_data))
...
for (lr in lr_values) {
  for (dropout in dropout_values) {
    # Define the model architecture with current hyperparameters
    model <- nn_sequential(
      nn_linear(97, 4),
      nn_relu(),
