...
# Load the data
#setwd("media/thokas/Yeah/Acute_Lymphoid_Leukemia_Project-main/Code/ML/ALLDEGs")
#setwd("D:/Acute_Lymphoid_Leukemia_Project-main/Code/ML/ALLDEGs/")
# Read the CPM table, set the first column as row names, and transpose the data
ML_data <- fread("../../ALLDEGs/cpm_table_log_HS.csv") %>% 
  tibble() %>% 
...
recipe <- recipe(type ~ ., data = train_data)  %>%
  ...
  step_bsmote(type,over_ratio = 0.5,seed = 1234,all_neighbors = T) %>%
  #step_adasyn(type,over_ratio = 0.5) %>%
  step_tomek(type)
# Prepare the recipe
rec_prep <- prep(recipe, training = train_data)

# Apply the recipe to the training data and view transformed data
baked_data <- bake(rec_prep, new_data = NULL)
table(baked_data$type)
# Perform cross-validation
cv_folds <- vfold_cv(train_data, strata = type, v = 3, repeats = 2)

```

```{r SMOTE Split tidymodels, warning=FALSE}
set.seed(1234)

# For K-fold cross validation
data_split <- initial_split(my_data_train, strata = type, prop = 0.8)
train_data <- training(data_split)
test_data <- testing(data_split)

# Create a data split
smote_data <- RSLS(train_data[-ncol(train_data)], train_data$type, dupSize = 1,K = 5, C = 5)
#smote_data <- DBSMOTE(my_data_train[-ncol(my_data_train)], my_data_train$type, dupSize = 1)
...
train_data <- rbind(train_data, smote_data)
...
recipe <- recipe(type ~ ., data = train_data) %>%
  ...
  step_tomek(type)
  #step_ns(all_numeric(),deg_free = 5)  # Add natural splines for flexibility

# Detailed summary of the recipe
summary(recipe)
# Prepare the recipe
rec_prep <- prep(recipe, training = train_data)
...
rf_spec <- 
  ...
  set_engine("ranger", importance = "impurity") # permutation impurity
...
tune_2 <- tune_sim_anneal(tuned_rf_workflow,resamples = cv_folds, iter = 10, initial = tune_1)
...
RF_imp_plot <- ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  labs(title = "Variable Importance Plot RF HS AGE Batch", x = "Importance", y = "Variable") +
...
RF_imp_plot
#ggsave(filename = "Variable_Importance_Plot_RF_HS_AGE_Batch.png", plot = RF_imp_plot, dpi = 300,bg = "white")
...
# Define the search grid for tuning
xgb_grid <- grid_space_filling(draft,size = 15,type = "latin_hypercube" )
...
tune_2 <- tune_sim_anneal(tuned_xgb_workflow, resamples = cv_folds, iter = 10, initial = tune_1)
...
xgb_importance <- xgb.ggplot.importance(importance_matrix = importance_matrix, top_n = 25, n_clusters = 4,rel_to_first = F) + 
  ggtitle("Feature Importance XGBoost HS AGE Batch")
xgb_importance
# Save the plot with high DPI
#ggsave(filename = "Variable_Importance_Plot_XGB_HS_AGE_Batch.png", plot = xgb_importance, dpi = 300,bg = "white")
...
mlp_spec <- mlp(hidden_units = c(128, 64, 32), 
                penalty = tune(), 
                epochs = tune(), 
                learn_rate = tune(), 
                activation = c("relu", "selu", "gelu")) %>% 
  set_mode("classification") %>% 
...
tune_1 <- tune_grid(tuned_svm_workflow,resamples = cv_folds, grid = mlp_grid)

tune_2 <- tune_sim_anneal(tuned_svm_workflow, resamples = cv_folds, iter = 30, initial = tune_1)
...
tune_2 <- tune_sim_anneal(tuned_discrim_linear_workflow, resamples = cv_folds, iter = 30, initial = tune_1)
...
extract_workflow(discrim_linear_fit)
...
all_metrics <- bind_rows(RF_tuned_metrics, Mlp_tuned_metrics,LDA_tuned_metrics,SVM_tuned_metrics,XGB_tuned_metrics)
...
write.csv(all_metrics,"Metrics_HS_AGE_batch.csv")

```

## Print plot

```{r dot_plot, warning=FALSE, echo=FALSE}
dot_plot <- all_metrics %>% 
  ...
  labs(title = "Model Performance Metrics HS AGE Batch", x = "Metric", y = "Value") + 
  theme_minimal(base_size = 10) + 
  theme(legend.position = "top") + 
  scale_color_brewer(palette = "Dark2")
print(dot_plot)
# Save the plot with high DPI
ggsave(filename = "Metrics_plot_HS_AGE_Batch.png", plot = dot_plot, dpi = 300,bg = "white")
