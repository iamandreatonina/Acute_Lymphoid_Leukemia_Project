...
ML_data <- read_csv("../../ALLDEGs/cpm_table_log_HS_nobatch.csv") %>% 
...
recipe <- recipe(type ~ ., data = train_data)  %>%
  ...
  step_adasyn(type,over_ratio = 1) #%>%
  #step_ns(all_predictors(),deg_free = 5) #%>%
  #step_smotenc(type, over_ratio = 0.5)
...
rf_spec <- 
  ...
  set_engine("ranger", num.threads = 10, importance = "impurity") #permutation
...
RF_imp_plot <- ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  labs(title = "Variable Importance Plot RF HS AGE", x = "Importance", y = "Variable") +
...
ggsave(filename = "Variable_Importance_Plot_RF_HS_AGE.png", plot = RF_imp_plot, dpi = 300,bg = "white")
...
xgb_importance <- xgb.ggplot.importance(importance_matrix = importance_matrix, top_n = 25, n_clusters = 4,rel_to_first = F) + 
  ggtitle("Feature Importance XGBoost HS AGE")
# Save the plot with high DPI
ggsave(filename = "Variable_Importance_Plot_XGB_HS_AGE.png", plot = xgb_importance, dpi = 300,bg = "white")
...
B_nnet_imp_plot <- ggplot(head(bagged_nnet_imp_2, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  geom_bar(stat = "identity") + # Use bar chart to display importance
  scale_fill_manual(values = c("lightcoral", "lightgreen", "skyblue")) +  # Choose colors for each category
  labs(title = "Variable Importance Plot B_nnet HS AGE", x = "Importance", y = "Variable") +
  theme_minimal() + # Use classic theme for clean appearance
  theme(axis.text.y = element_text(size = 10), # Customize y-axis text size
        plot.title = element_text(size = 15), # Customize plot title size
        legend.position = "right") + # Position the legend on the right
  guides(fill = guide_legend(reverse = TRUE, title.position = "top")) # Reverse legend order
ggsave(filename = "Variable_Importance_Plot_B_nnet_HS_AGE.png", plot = B_nnet_imp_plot, dpi = 300,bg = "white")
...
tune1 <- tune_grid(tuned_svm_workflow,resamples = cv_folds, grid = mlp_grid)

tune2 <- tune_sim_anneal(tuned_svm_workflow, resamples = cv_folds, iter = 25, initial = tune1, metrics = class_metrics)
...
draft <-extract_parameter_set_dials(discrim_linear) %>%
  finalize(train_data)
# Define the search grid for tuning
discrim_linear_grid <- grid_space_filling(draft,size = 15, type = "latin_hypercube")
...
tune1 <- tune_grid(tuned_discrim_linear_workflow,resamples = cv_folds, grid = discrim_linear_grid)

tune2 <- tune_sim_anneal(tuned_discrim_linear_workflow, resamples = cv_folds, iter = 30, initial = tune1, metrics = class_metrics)
...
write.csv(all_metrics,"Metrics_HS_AGE.csv")
...
dot_plot <- all_metrics %>% 
  ...
  labs(title = "Model Performance Metrics HS AGE", x = "Metric", y = "Value") + 
  theme_minimal(base_size = 10) + 
  theme(legend.position = "top") + 
  scale_color_brewer(palette = "Dark2")
print(dot_plot)
# Save the plot with high DPI
ggsave(filename = "Metrics_plot_HS_AGE.png", plot = dot_plot, dpi = 300,bg = "white")
