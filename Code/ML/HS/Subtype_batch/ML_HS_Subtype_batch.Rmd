...
ML_data <- read_csv("../../ALLDEGs/cpm_table_log_HS.csv") %>%
...
set.seed(1234)
# Create a data split
smote_data <- ANS(my_data_train[-ncol(my_data_train)], my_data_train$Cell_type, dupSize = 6)
...
recipe <- recipe(Cell_type ~ ., data = train_data) %>%  
  ...
  step_ns(all_predictors(),deg_free = 5)  # Add natural splines for flexibility
...
rf_spec <- 
  ...
  set_engine("ranger", importance = "permutation") #impurity
...
# Define the search grid for tuning
tree_grid <- grid_space_filling(draft,size = 5)
...
RF_imp_plot <- ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  labs(title = "Variable Importance Plot RF HS Subtype Batch", x = "Importance", y = "Variable") +
...
ggsave(filename = "Variable_Importance_Plot_RF_HS_Subtype_Batch.png", plot = RF_imp_plot, dpi = 300,bg = "white")
...
# Define the search grid for tuning
Knn_grid <- grid_space_filling(draft,size = 15)
...
# Define the search grid for tuning
xgb_grid <- grid_space_filling(draft,size = 15)
...
ggsave(filename = "Variable_Importance_Plot_XGB_HS_Subtype_Batch.png", plot = xgb_importance, dpi = 300,bg = "white")
...
# Define thresholds for categorizing variable importance
high_threshold <- max(bagged_nnet_imp_2$importance) * 0.5
medium_threshold <- max(bagged_nnet_imp_2$importance) * 0.2
...
B_nnet_imp_plot <- ggplot(head(bagged_nnet_imp_2, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  geom_bar(stat = "identity") + # Use bar chart to display importance
  scale_fill_manual(values = c("lightcoral", "lightgreen", "skyblue")) +  # Choose colors for each category
  labs(title = "Variable Importance Plot B_nnet HS Subtype Batch", x = "Importance", y = "Variable") +
  theme_minimal() + # Use classic theme for clean appearance
  theme(axis.text.y = element_text(size = 10), # Customize y-axis text size
        plot.title = element_text(size = 15), # Customize plot title size
        legend.position = "right") + # Position the legend on the right
  guides(fill = guide_legend(reverse = TRUE, title.position = "top")) # Reverse legend order
ggsave(filename = "Variable_Importance_Plot_B_nnet_HS_Subtype_Batch.png", plot = B_nnet_imp_plot, dpi = 300,bg = "white")
...
write_csv(bagged_nnet_imp_2,"Mlp_tuned_importance")
...
all_metrics <- all_metrics %>%
  ...
  arrange(-`F-score`)
all_metrics
write_csv(all_metrics,"Metrics_HS_subtype_batch.csv")
...
dot_plot <- all_metrics %>% 
  ...
  geom_jitter(size = 3, alpha = 0.8, height = 0.1) +  # Add vertical jitter to avoid overlap
...
print(dot_plot)
# Save the plot with high DPI
ggsave(filename = "Metrics_HS_subtype_Batch.png", plot = dot_plot, dpi = 300,bg = "white")
