...
ML_data <- fread("../../ALLDEGs/cpm_table_log_HS_nobatch.csv") %>%
  tibble() %>%
...
set.seed(1234)
# Create a data split
smote_data <- RSLS(my_data_train[-ncol(my_data_train)], my_data_train$Cell_type, dupSize = 8,K = 5, C = 5)
...
agmented_data <- rbind(my_data_train, smote_data)
table(train_data$Cell_type)

# For K-fold cross validation
data_split <- initial_split(agmented_data, strata = Cell_type, prop = 0.7)
...
# Define the search grid for tuning
tree_grid <- grid_space_filling(draft,size = 15, type = "latin_hypercube")
...
tune_2 <- tune_sim_anneal(tuned_rf_workflow,resamples = cv_folds, iter = 2, initial = tune_1, metrics = class_metrics)
...
RF_imp_plot <- ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  labs(title = "Variable Importance Plot RF HS Subtype", x = "Importance", y = "Variable") +
...
RF_imp_plot
#ggsave(filename = "Variable_Importance_Plot_RF_HS_Subtype.png", plot = RF_imp_plot, dpi = 300,bg = "white")
...
# Define the search grid for tuning
Knn_grid <- grid_space_filling(draft,size = 15, type = "latin_hypercube")
...
draft <- extract_parameter_set_dials(xgb_spec) %>% finalize(train_data)
xgb_grid <- grid_space_filling(draft, size = 15, type = "latin_hypercube")

# Tuning Workflow
tuned_xgb_workflow <- workflow() %>%
  add_recipe(recipe) %>%
  add_model(xgb_spec)
...
# Collect Tuning Results
tuning_results <- show_best(tune_2, metric = "accuracy")
best_params <- select_best(tune_2, metric = "accuracy")

# Finalize and Fit the Model
xgb_model_final <- finalize_model(xgb_spec, best_params)
xgb_fit <- xgb_model_final %>% last_fit(recipe, split = data_split)
...
xgb_importance <- xgb.ggplot.importance(importance_matrix = importance_matrix, top_n = 25, n_clusters = 4,rel_to_first = F) + 
  ggtitle("Feature Importance XGBoost HS Subtype")
# Save the plot with high DPI
ggsave(filename = "Variable_Importance_Plot_XGB_HS_Subtype.png", plot = xgb_importance, dpi = 300,bg = "white")
...
mlp_spec <- mlp(hidden_units = c(64, 32, 16), 
                penalty = tune(), 
                epochs = tune(), 
                learn_rate = tune(), 
...
# Search Grid
mlp_grid <- grid_space_filling(draft, size = 15, type = "latin_hypercube")
...
# Create a bar plot of the top 25 variable importances
#bagged_nnet_imp_2 <- fread("Mlp_tuned_importance.csv")
B_nnet_imp_plot <- ggplot(head(bagged_nnet_imp_2, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  geom_bar(stat = "identity") + # Use bar chart to display importance
  scale_fill_manual(values = c("lightcoral", "lightgreen", "skyblue")) +  # Choose colors for each category
  labs(title = "Variable Importance Plot B_nnet HS Subtype", x = "Importance", y = "Variable") +
  theme_minimal() + # Use classic theme for clean appearance
  theme(axis.text.y = element_text(size = 10), # Customize y-axis text size
        plot.title = element_text(size = 15), # Customize plot title size
        legend.position = "right") + # Position the legend on the right
  guides(fill = guide_legend(reverse = TRUE, title.position = "top")) # Reverse legend order
ggsave(filename = "Variable_Importance_Plot_B_nnet_HS_Subtype.png", plot = B_nnet_imp_plot, dpi = 300,bg = "white")
...
all_metrics <- bind_rows(RF_tuned_metrics, Mlp_tuned_metrics, KNN_tuned_metrics,XGB_tuned_metrics)
...
write.csv(all_metrics,"Metrics_HS_Subtype.csv")
...
dot_plot <- all_metrics %>%
  ...
  labs(title = "Model Performance Metrics HS Subtype", x = "Metric", y = "Value") +
...
# Save the plot with high DPI
ggsave(filename = "Metrics_HS_subtype.png", plot = dot_plot, dpi = 300,bg = "white")
