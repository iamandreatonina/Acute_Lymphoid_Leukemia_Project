...
RF_imp_plot <- ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  labs(title = "Variable Importance Plot RF nonHS Subtype Batch", x = "Importance", y = "Variable") +
...
ggsave(filename = "Variable_Importance_Plot_RF_nonHS_Subtype_Batch.png", plot = RF_imp_plot, dpi = 300,bg = "white")
...
xgb_importance <- xgb.ggplot.importance(importance_matrix = importance_matrix, top_n = 25, n_clusters = 4,rel_to_first = F) + 
  ggtitle("Feature Importance XGBoost nonHS Subtype Batch")
# Save the plot with high DPI
ggsave(filename = "Variable_Importance_Plot_XGB_nonHS_Subtype_Batch.png", plot = xgb_importance, dpi = 300,bg = "white")
...
## results Bagged Neural Network with Baguette(nnet)
...
all_metrics <- all_metrics %>%
  ...
  arrange(-`F-score`)
all_metrics
write_csv(all_metrics,"Metrics_nonHS_subtype_batch.csv")
...
print(dot_plot)
# Save the plot with high DPI
ggsave(filename = "MMetrics_plot_nonHS_subtype_batch.png", plot = dot_plot, dpi = 300,bg = "white")
