...
ML_data <- read_csv("../../ALLDEGs/cpm_table_log_woHS.csv") %>%
...
RF_imp_plot <- ggplot(head(RF_var_importance, 25), aes(x = importance, y = reorder(Ens_id, importance), fill = Category)) +
  ...
  labs(title = "Variable Importance Plot RF nonHS AGE Batch", x = "Importance", y = "Variable") +
...
ggsave(filename = "Variable_Importance_Plot_RF_nonHS_AGE_Batch.png", plot = RF_imp_plot, dpi = 300,bg = "white")
...
xgb_importance <- xgb.ggplot.importance(importance_matrix = importance_matrix, top_n = 25, n_clusters = 4,rel_to_first = F) + 
  ggtitle("Feature Importance XGBoost nonHS AGE Batch")
# Save the plot with high DPI
ggsave(filename = "Variable_Importance_Plot_XGB_nonHS_AGE_Batch.png", plot = xgb_importance, dpi = 300,bg = "white")
...
# Define the model specification with the ranger engine
mlp_spec <- mlp(hidden_units = 16, 
                penalty = tune(), 
                epochs = 200) %>%
  set_mode("classification") %>%
  set_engine("nnet", MaxNWts = 35282)

# Extract parameter set and finalize based on training data
draft <- extract_parameter_set_dials(mlp_spec) %>% 
  finalize(train_data)

# Define the search grid for tuning
mlp_grid <- grid_space_filling(draft, size = 25)

# Create the tuning workflow with the recipe and model specification
tuned_mlp_workflow <- workflow() %>%
  add_recipe(recipe) %>%
  add_model(mlp_spec)

# First tuning process with grid search
tune_1 <- tune_grid(tuned_mlp_workflow,
                   resamples = cv_folds, 
                   grid = mlp_grid)

# Simulated annealing tuning process
#tune_2 <- tune_sim_anneal(tuned_mlp_workflow, 
 #                        resamples = cv_folds, 
  #                       iter = 30, 
   #                      initial = tune_1, 
    #                     metrics = class_metrics)
...
# Finalize the model with the best hyperparameters
mlp_model_final <- finalize_model(x = mlp_spec, 
                                  parameters = best_params)

# Fit the model to the training data split
mlp_fit <- mlp_model_final %>% last_fit(recipe, split = data_split)
...
draft <-extract_parameter_set_dials(svm_linear) %>%
  finalize(train_data)
# Define the search grid for tuning
SVM_grid <- grid_space_filling(draft,size = 20, type = "latin_hypercube")
...
draft <-extract_parameter_set_dials(discrim_linear) %>%
  finalize(train_data)
# Define the search grid for tuning
discrim_linear_grid <- grid_space_filling(draft,size = 20, type = "latin_hypercube")
...
write.csv(all_metrics,"Metrics_nonHS_AGE_Batch.csv")
...
dot_plot <- all_metrics %>% 
  ...
  labs(title = "Model Performance Metrics nonHS AGE Batch", x = "Metric", y = "Value") + 
  theme_minimal(base_size = 10) + 
  theme(legend.position = "top") + 
  scale_color_brewer(palette = "Dark2")
# Save the plot with high DPI
ggsave(filename = "Metrics_plot_nonHS_AGE_Batch.png", plot = dot_plot, dpi = 300,bg = "white")
print(dot_plot)
```
